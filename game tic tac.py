...
class TicTacToe:
    def __init__(self):
        self.board = ['' for _ in range(9)]  # board 3x3
    ...
    def winner(self, square, letter):
        # winner if 3 in a row anywhere we have to check everything
        # check row
        row_ind = square // 3
        ...
        # check diagonals
        # but only if square is an even number (o, 2, 4, 6, 8)
        # these are the only moves possible to win a diagonals
        if square % 2 == 0:
            diagonal1 = [self.board[i] for i in [0, 4, 8]]  # lef to right diagonal
            if all([spot == letter for spot in diagonal1]):
                return True
            diagonal2 = [self.board[i] for i in [2, 4, 6]]  # right to lef diagonal
            if all([spot == letter for spot in diagonal2]):
                return True

        # if all of the checks failed we don't have a winner
        return False
